...
- 拥塞避免阶段：直到拥塞窗口的大小大于ssthresh时进入拥塞避免阶段。在拥塞避免阶段,cwnd一次增加一个MSS，进入线性增长的阶段。
- 快速恢复：直到出现了丢包，TCP就认为网络发生了拥塞，此时需要将ssthresh设置为此时cwnd的一半，而cwnd重设为1个MSS，重新进入慢启动阶段。



**15. 详细描述从接入网络到获取谷歌网站页面的全过程**
